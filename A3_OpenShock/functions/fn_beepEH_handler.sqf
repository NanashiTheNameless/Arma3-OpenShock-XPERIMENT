/*
    ...
    Description:
    Handles Toggle/Enable/Disable EH for OpenShock Beep Command.
...
switch (_state) do {
	...
	case 1: { // Enable
		...
		if ((player getVariable ["NUG_beepEHIndex", -1]) >= 0)  then {
			player removeEventHandler [(player getVariable ["NUG_beepEHType", -1]), (player getVariable ["NUG_beepEHIndex", -1])];
		...
		_EHIndex = player addEventHandler [NUG_beep_handler, {
			[round NUG_beep_duration] call NUG_fnc_beep;
		}];

		player setVariable ["NUG_beepEHIndex", _EHIndex];
		player setVariable ["NUG_beepEHType", NUG_beep_handler];
